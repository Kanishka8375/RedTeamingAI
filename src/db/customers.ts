...
export function createCustomer(name: string, apiKey: string, plan: Plan = 'free'): Customer {
  ...
  db.prepare(`INSERT INTO customers(id, api_key, name, plan, monthly_event_limit, created_at, blocked)
    VALUES(@id, @api_key, @name, @plan, @monthly_event_limit, @created_at, @blocked)`).run({
    id: customer.id,
