...
type MockResponse = {
  ...
  set: (field: string | Record<string, string>, value?: string) => MockResponse;
  setHeader: (key: string, value: string | number | readonly string[]) => MockResponse;
  ...
  const response: MockResponse = {
    ...
    set(field, value) {
      if (typeof field === 'string') {
        ...
        Object.entries(field).forEach(([k, v]) => headers.set(k, v));
      }
      return response;
    },
    setHeader(key, value) {
...
describe('proxy pricing and interceptor guardrails', () => {
  ...
  it('returns 401 when API key is missing', async () => {
    ...
    await interceptor(req, res as unknown as Response, (() => undefined) as NextFunction);
